...
namespace Synthborn.Combat
{
    ...
    public class AutoAttackController : MonoBehaviour
    {
        ...
        [SerializeField] private ObjectPool<ProjectileController> _projectilePool;
        ...
        private void Update()
            ...
            for (int i = 0; i < _slots.Count; i++)
            {
                ...
                if (state.CooldownRemaining <= 0f)
                {
                    ...
                    var target = _targeting.FindBestTarget(origin, facing, _config.attackRange, _config.targetingConeAngle);
                    if (target != null)
                    {
                        FireProjectile(origin, target.position, slotData);
                        float effectiveInterval = Mathf.Max(
                            slotData.baseInterval * (1f - Mathf.Clamp(_stats.AttackSpeedModifier, 0f, _config.attackSpeedModifierMax)),
                            0.1f);
        ...
        private void FireProjectile(Vector2 origin, Vector2 targetPos, AttackSlotData slotData)
            ...
            var proj = _projectilePool.Get(origin);
            ...
            float critMult = slotData.projectileData.baseCritMultiplier + _stats.CritMultiplier;
