...
{
    /// <summary>
    /// Between-level shop: buy consumables, reroll loot, buy materials.
    ...
    public class ShopScreen : MonoBehaviour
    {
        ...
        private void Refresh()
        {
            ...
            // Shop items
            CreateShopItem("Scrap Metal x5", "Raw crafting material", 50, () => {
                ch.scrapMetal += 5;
            });

            CreateShopItem("Dark Crystal x2", "Rare crafting material", 150, () => {
                ch.darkCrystals += 2;
            });

            CreateShopItem("Health Potion", "Restore 50% HP at level start", 100, () => {
                // Store as a flag — GameBootstrap checks and heals at run start
                PlayerPrefs.SetInt("ShopHealPotion", 1);
                PlayerPrefs.Save();
            });

            CreateShopItem("XP Scroll", "+50% XP for next level", 200, () => {
                PlayerPrefs.SetInt("ShopXPScroll", 1);
                PlayerPrefs.Save();
            });

            CreateShopItem("Lucky Coin", "+25% drop rate for next level", 250, () => {
        ...
        private void CreateShopItem(string name, string desc, int cost, System.Action onBuy)
        {
            ...
            text.text = $"<b>{name}</b>  <color=yellow>{cost}g</color>\n<size=10>{desc}</size>";
            ...
            bt.text = "BUY"; bt.fontSize = 14; bt.color = canBuy ? Color.white : Color.gray;
            ...
            if (canBuy)
            {
                ...
                btnGO.GetComponent<Button>().onClick.AddListener(() =>
                {
                    if (ch.gold >= capturedCost)
                    {
                        ...
                        if (_resultText != null) _resultText.text = $"<color=green>Purchased {name}!</color>";
                        Refresh();
                    }
                });
            }
