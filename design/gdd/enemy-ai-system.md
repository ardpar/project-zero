...
**Boss HP Cross-Validation (Worked Example — Wave 7 Boss):**
```
boss_base_hp       = 50        (from BossData ScriptableObject)
tier_multiplier    = 10        (Boss tier)
wave_scaling       = (1 + 7 * 0.15) = 2.05
effective_boss_hp  = 50 * 10 * 2.05 = 1025 HP
```
- Expected player DPS at level 15 (late game): ~25 damage/sec
- Boss fight duration: 1025 / 25 = ~41 seconds
- Total run time: ~690s (waves 1-7 spawning) + 41s (boss fight) = ~731s ≈ 12 minutes
- **Verdict:** Within 10-15 minute run target. Boss fight is ~5.6% of total run time — feels like a climactic cap, not a slog.
